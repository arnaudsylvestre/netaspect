       ...
       protected override Action<List<ErrorReport.Error>> CreateErrorHandlerProvider()
       {
           return errorHandler => errorHandler.Add(
              new ErrorReport.Error
              {
                  ...
                  Message = string.Format("The parameter 'unknown' in the interceptor AfterCallMethod of the aspect {0} is unknown. Expected one of : called, calledparameters, calledv, caller, callerparameters, callermethod, columnnumber, linenumber, filepath, filename, result", typeof(LogAttribute).FullName)
              });
       }
