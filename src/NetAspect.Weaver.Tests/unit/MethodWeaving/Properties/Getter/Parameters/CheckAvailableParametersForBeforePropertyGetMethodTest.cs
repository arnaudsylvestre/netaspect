        ...
        protected override Action<List<ErrorReport.Error>> CreateErrorHandlerProvider()
        {
            return errorHandler => errorHandler.Add(
               new ErrorReport.Error
               {
                   ...
                   Message = string.Format("The parameter 'unknown' in the interceptor BeforePropertyGetMethod of the aspect {0} is unknown. Expected one of : instance, property, linenumber, columnnumber, filename, filepath", typeof(LogAttribute).FullName)
               });
        }
