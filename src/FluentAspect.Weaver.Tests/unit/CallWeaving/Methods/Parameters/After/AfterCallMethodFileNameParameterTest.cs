...
{
    [TestFixture]
    public class AfterCallMethodFileNameParameterTest 
    {
         [Test]
         public void CheckCallMethodWithFileNameAndNoDebuggingInformation()
         {
          DoUnit.Test(new ClassAndAspectAndCallAcceptanceTestBuilder())
                 .ByDefiningAssembly(simpleClassAndWeaver =>
                    ...
                    .EnsureErrorHandler(e => e.Warnings.Add("The parameter fileName in method AfterCallMethodValue of type A.MyAspectAttribute will have the default value because there is no debugging information"))
                  .AndEnsureAssembly((assembly, actual) =>
                      {
                          var caller = actual.CreateCallerObject();
                          actual.CallCallerMethod(caller);
                          Assert.AreEqual(0, actual.Aspect.AfterCallMethodValueFileName);
                      })
                  .AndLaunchTest();
         }
         ...
         public void CheckCallMethodWithFileNameAndNoDebuggingInformationAndWrongType()
         {
             ...
             //       })
             //          .EnsureErrorHandler(e => e.Errors.Add("The parameter fileName in method AfterCallMethodValue of type A.MyAspectAttribute will have the default value because there is no debugging information"))
             //        .AndEnsureAssembly((assembly, actual) =>
             //        {
             //            var caller = actual.CreateCallerObject();
             //            actual.CallCallerMethod(caller);
