...
{
    [TestFixture]
    public class AfterCallEventFilePathParameterTest 
    {
         [Test]
         ...
         public void CheckCallEventWithFilePathAndNoDebuggingInformationAndWrongType()
         {
             ...
             //       })
             //          .EnsureErrorHandler(e => e.Errors.Add("The parameter filePath in method AfterCallEventValue of type A.MyAspectAttribute will have the default value because there is no debugging information"))
             //        .AndEnsureAssembly((assembly, actual) =>
             //        {
             //            var caller = actual.CreateCallerObject();
             //            actual.CallCallerMethod(caller);
