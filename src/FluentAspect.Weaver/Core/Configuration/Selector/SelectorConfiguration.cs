...
namespace FluentAspect.Weaver.Core.Configuration.Selector
{
    public class SelectorConfigurationReader : IConfigurationReader
    {
        ...
        {
            foreach (Type attribute in types)
            {
                var attribute_L = new NetAspectDefinition(attribute);
                if (attribute_L.SelectorMethod != null)
                {
                    ...
                    configuration.AddMethod(m => CheckMethod(m, attribute_L.SelectorMethod), GetAssemblies(attribute_L),
                                            attribute_L, errors);
                    errorHandler.Errors.AddRange(errors);
                }
                if (attribute_L.SelectorConstructor != null)
                {
                    ...
                    if (errors.Count > 0)
                        continue;
                    errors = new List<string>();
                    configuration.AddConstructor(m => CheckMethod(m, attribute_L.SelectorConstructor),
                                                 GetAssemblies(attribute_L), attribute_L, errors);
                    errorHandler.Errors.AddRange(errors);
                }
            }
        }
        ...
        {
            ParametersFiller filler = CreateFiller();
            filler.SetValue("methodName", arg.Name);
            List<object> parameters = filler.Fill(info);
        ...
        {
            private readonly Dictionary<string, Parameter> descriptions = new Dictionary<string, Parameter>();
            ...
            public void Check(MethodInfo info, List<string> handler)
            {
                ...
                {
                    string fullName = descriptions[parameterInfo.Name].Type.FullName;
                    if (fullName != parameterInfo.ParameterType.FullName)
                        handler.Add(string.Format("Wrong type for parameter {0}, expected {1}", parameterInfo.Name,
                                                  fullName));
                }
            }
