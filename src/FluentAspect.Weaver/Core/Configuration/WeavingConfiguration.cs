...
namespace FluentAspect.Weaver.Core.Configuration
{
    ...
    public class WeavingConfiguration
    {
        ...
        private readonly List<AspectMatch<IMethod>> _constructors;
        private readonly List<AspectMatch<FieldReference>> _events;
        private readonly List<AspectMatch<FieldReference>> _fields;
        private readonly List<AspectMatch<IMethod>> _methods;
        private readonly List<AspectMatch<ParameterDefinition>> _parameters;
        private readonly List<AspectMatch<PropertyReference>> _properties;
        ...
        public IEnumerable<AssemblyDefinition> AllAssemblies
        {
            get
            {
                var assemblies_L = new List<AssemblyDefinition>();
        ...
        {
            if (methodWeavingConfigurations != null &&
                methodWeavingConfigurations.Type.GetConstructor(new Type[0]) == null)
                errors.Add(string.Format("The type {0} must have a default constructor",
                                         methodWeavingConfigurations.Type.FullName));
        }
