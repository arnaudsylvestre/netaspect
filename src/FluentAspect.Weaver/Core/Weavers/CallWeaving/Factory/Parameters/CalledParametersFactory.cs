...
namespace FluentAspect.Weaver.Core.Weavers.CallWeaving.Factory.Parameters
{
    ...
    {
        public static void AddCalledParameters(this ParametersEngine engine, MethodReference reference_P,
                                               Dictionary<string, VariableDefinition> variablesForParameters)
        {
            foreach (ParameterDefinition parameterDefinition_L in reference_P.Parameters.Reverse())
            {
                engine.AddPossibleParameter((parameterDefinition_L.Name + "Called").ToLower(), (info, handler) => { },
                                            (info, instructions) =>
                                                {
                                                    VariableDefinition variable =
                                                        variablesForParameters[
                                                            info.Name.Substring(0, info.Name.Length - "Called".Length)];
