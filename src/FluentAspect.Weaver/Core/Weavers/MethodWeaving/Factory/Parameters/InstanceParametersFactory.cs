    ...
    public static class InstanceParametersFactory
    {
                                        ...
                                        (p, handler) =>
                                            {
                                                ...
                                                Ensure.OfType(p, handler, typeof (object).FullName,
                                     ...
                                     VariableDefinition result)
        {
            engine.AddPossibleParameter("result",
                                        (p, handler) => { Ensure.ResultOfType(p, handler, methodDefinition); },
                                        (info, instructions) =>
                                        instructions.Add(
                                            Instruction.Create(
                                                info.ParameterType.IsByRef ? OpCodes.Ldloca : OpCodes.Ldloc, result))
                );
        }
        ...
        public static void AddException(this ParametersEngine engine, VariableDefinition exception)
        {
            engine.AddPossibleParameter("exception",
                                        ...
                                        (info, instructions) =>
                                            { instructions.Add(Instruction.Create(OpCodes.Ldloc, exception)); }
                );
        }
        ...
        public static void AddParameterNames(this ParametersEngine engine, MethodDefinition methodDefinition,
            ...
            foreach (ParameterDefinition parameter in methodDefinition.Parameters)
            {
                try
                {
                    ...
                    engine.AddPossibleParameter(parameter1.Name,
                                                (p, handler) => { Ensure.OfType(p, handler, parameter1); },
                                                (p, _instructions) =>
                                                    {
                                                        ModuleDefinition moduleDefinition =
                                                            ((MethodDefinition) parameter.Method).Module;
                                                        if (p.ParameterType.IsByRef &&
                                                            !parameter.ParameterType.IsByReference)
                                                        {
                                                            _instructions.Add(Instruction.Create(OpCodes.Ldarga,
                                                                                                 parameter));
                                                        }
                                                        else if (!p.ParameterType.IsByRef &&
                                                                 parameter.ParameterType.IsByReference)
                                                        {
                                                            _instructions.Add(Instruction.Create(OpCodes.Ldarg,
                                                                                                 parameter));
                                                            _instructions.Add(Instruction.Create(OpCodes.Ldobj,
                                                                                                 moduleDefinition
                                                                                                     .Import(
                                                                                                         p
                                                                                                             .ParameterType)));
                                                            ...
                                                            p.ParameterType == typeof (Object))
                                                        {
                                                            ...
                                                            {
                                                                reference =
                                                                    ((MethodDefinition) parameter.Method)
                                                                        .GenericParameters.First(
