...
namespace FluentAspect.Weaver.Core.WeaverBuilders
{
    public class CallMethodBuilderWeaver : IWeaverBuilder
    {
        public IEnumerable<IWeaveable> BuildWeavers(WeavingConfiguration configuration)
        {
            ...
            {
                foreach (AssemblyDefinition assemblyDefinition in m.AssembliesToScan)
                {
                    ...
                    foreach (MethodDefinition method in methods)
                    {
                        ...
                        foreach (Instruction instruction in method.Body.Instructions)
                        {
                                ...
                                instruction.OpCode == OpCodes.Newobj && instruction.Operand is MethodReference)
                            {
                                ...
                                AspectMatch<IMethod> methodMatch = m;
                                {
                                    if (
                                        methodMatch.Matcher(
                                            new MethodDefinitionAdapter(instruction.Operand as MethodReference)))
                                    {
            ...
            return
                points.Select(
                    point_L => new AroundInstructionWeaver(point_L.Key, new CallMethodWeaver(new MethodCallToWeave
                        {
