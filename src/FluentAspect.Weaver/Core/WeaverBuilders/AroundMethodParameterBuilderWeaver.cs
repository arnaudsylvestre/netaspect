        ...
        public IEnumerable<IWeaveable> BuildWeavers(WeavingConfiguration configuration)
        {
            ...
            foreach (var methodMatch in configuration.Parameters)
            {
                foreach (AssemblyDefinition assemblyDefinition in methodMatch.AssembliesToScan)
                {
                    ...
                    foreach (MethodDefinition methodDefinition_L in methods)
                    {
                        foreach (ParameterDefinition parameterDefinition in methodDefinition_L.Parameters)
                        {
            ...
            return
                configurations.Select(
                    configuration_L =>
                    new AroundMethodWeaver(configuration_L.Value.Select(a => a.Configuration).ToList(),
                                           configuration_L.Key, weave_P => new ParameterWeaver(weave_P, null)))
                              .Cast<IWeaveable>();
        }
