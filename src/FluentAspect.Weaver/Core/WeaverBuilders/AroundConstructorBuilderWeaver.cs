        ...
        public IEnumerable<IWeaveable> BuildWeavers(WeavingConfiguration configuration)
        {
            ...
            foreach (var methodMatch in configuration.Constructors)
            {
                foreach (AssemblyDefinition assemblyDefinition in methodMatch.AssembliesToScan)
                {
                    ...
                    {
                        if (methodMatch.Matcher(new MethodDefinitionAdapter(methodDefinition_L)) &&
            ...
            return
                configurations.Select(
                    configuration_L => new AroundConstructorWeaver(configuration_L.Value, configuration_L.Key))
                              .Cast<IWeaveable>();
        }
    }
}